...
private:
    ...
    void createSurface() {
        ...
        if (xcb_connection_has_error (xcb_connection))
          {
            throw std::runtime_error("Unable to initialize xcb connection");
          }
        ...
        if (!get_xcb_presentation_support(physicalDevice, 0,
                                          xcb_connection,
                                          iter.data->root_visual)) {
        ...
        create_xcb_surface(instance, &createSurfaceInfo, NULL, &surface);
    }
    ...
    void createGraphicsPipeline() {
        ...
        if (getenv("SRT_DATA_PATH"))
          {
            shadersPath = getenv("SRT_DATA_PATH");
          }
        else
          {
              shadersPath = argv0;
